    ...
    public PageInfo<UserInfoVo> findPage(String name, Integer pageNum, Integer pageSize, HttpServletRequest request) {
        ...
        for (int i = 0;i<all.size();i++){
            if (all.get(i).getStatus()==1){
                all.get(i).setStatusName("在职");
            }else {
                all.get(i).setStatusName("离职");
...
//            throw new CustomException(ResultCode.USER_ACCOUNT_ERROR);
    ...
    public UserInfo selectKaoQinInfo(UserInfo userInfo){
        return userInfoDao.selectKaoQinInfo(userInfo);
    }
