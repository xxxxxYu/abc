...
public class MenuController {
    ...
    public String getMenu(HttpServletRequest request) {
		...
		if (1 == level) {
			...
			array.add(getJsonObject("userInfo", "员工信息", "layui-icon-table", "userInfo"));
			array.add(getJsonObject("messageInfo", "留言信息", "layui-icon-table", "messageInfo"));
			array.add(getJsonObject("canshuInfo", "考勤参数信息", "layui-icon-table", "canshuInfo"));
			array.add(getJsonObject("kaoqinInfo", "考勤信息", "layui-icon-table", "kaoqinInfo"));
			array.add(getJsonObject("qingjiaInfo", "请假申请信息", "layui-icon-table", "qingjiaInfo"));
			array.add(getJsonObject("xiujiaInfo", "休假申请信息", "layui-icon-table", "xiujiaInfo"));
    ...
    public Result<Map<String, Integer>> getTotle() {
		...
		resultMap.put("canshuInfo", canshuInfoService.findAll().size());
		resultMap.put("kaoqinInfo", kaoqinInfoService.findAll().size());
